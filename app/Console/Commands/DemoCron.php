...
class DemoCron extends Command
{
    ...
    public function handle()
    {
        ...
        $flag = false;
        \Log::info("======Cron DailyUpdate working at ".$mytime."======");
        foreach ($trip as $trip_key) {
            $now = Carbon::today();
            $now = $now->addDay();

            $now_dayofweek = $now->dayOfWeek + 1;
            ...
            if (($now->gte($newDate) && $now->lte($newLastDate) && $dayflag) || ($now->eq($newDate) && $now->eq($newLastDate))) {
                ...
                foreach ($tripbus_data as $tripbus_key) {
                    \Log::info("======Cron DailyUpdate daily_trip create ".$trip_key->id."--".$now."======");
                    $daily_trip = new DailyTripDetail;
                    $daily_trip->trip_name = $trip_key->trip_name_en;

                    $temp = DailyTripDetail::where('start_date', $now->toDateString())->get();
                    $trip_id_str = $now->format('Y') . $now->format('m') . $now->format('d') . '-' . (count($temp) + 1);
                    ...
                    if(Area::where('id', $trip_key->destination_area)->first()){
                        $daily_trip->destination_area = Area::where('id', $trip_key->destination_area)->first()->area_name_en;
                    } else {
                        continue;
                    }
                    
                    if(City::where('id', $trip_key->destination_city)->first())
                    {
                    ...
                    $daily_trip->start_date = $now->toDateString();
                    $daily_trip->start_time = $trip_key->departure_time;
                    // $daily_trip->start_time = $mytime;

                    $daily_trip->end_date = $now->toDateString();
                    ...
                    $exist_dailyTrip = DailyTripDetail::where('start_date', $now->toDateString())
                    ...
                    $daily_trip->save();
                    \Log::info("======Cron DailyUpdate daily_trip create saved".$daily_trip->id."======");
                }
            }
        }
        $this->info('Successfully sent demo cron to everyone.');
    }
}
