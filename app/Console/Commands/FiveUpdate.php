...
class FiveUpdate extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'Five Command description';
    ...
    {
        \Log::info("Five Cron is working fine!");
        $today = Carbon::today()->toDateString();
        \Log::info("======-----Cron NewUpdate work at ".$today."-----======");        
        $newtrips = Trip::whereDate('created_at', $today)->get();
        \Log::info("======-----NewUpdate newTrips count = ".count($newtrips)."-----======");        
        if (count($newtrips) == 0) {
            \Log::info("======-----NewUpdate return count = ".count($newtrips)."-----======");
            $this->info('Successfully sent five cron to everyone.');
        }
...
\Log::info("!!!======-----NewUpdate create dailyTripDetail = ".$now->gte($newDate).",".$now->lte($newLastDate).",".$dayflag.",".$now->eq($newDate).",". $date.",".$now->eq($newLastDate).",".$newLastDate."-----======!!!");

            if (($now->gte($newDate) && $now->lte($newLastDate) && $dayflag) || ($now->eq($newDate) && $now->eq($newLastDate))) {
                \Log::info("Okay cron work");
                $tripbus_data = TripBus::where('trip_name', $trip_key->id)->get();
                foreach ($tripbus_data as $tripbus_key) {
                    \Log::info("!!!======-----NewUpdate create dailyTripDetail = ".$trip_key->id."-----======!!!");
                    ...
                    if(!City::where('id', $trip_key->origin_city)->first())continue;
                    if(!Area::where('id', $trip_key->origin_area)->first())continue;
                    if(!Area::where('id', $trip_key->destination_area)->first())continue;
                    if(!City::where('id', $trip_key->destination_city)->first())continue;
                    ...
                    // }
                    $exist_dailyTrip = DailyTripDetail::where('start_date', Carbon::now()->toDateString())
                                                        ->where('f_trip_id', $trip_key->id)
                                                        ->where('start_time', $trip_key->departure_time)
                                                        ->where('f_bus_id', $bus_no->id)
                                                        ->where('driver_id',$tripbus_key->driver_name)
                                                        ->where('client_name',$trip_key->client_id)
                                                        ->get();
                    
                    if(count($exist_dailyTrip) >= 1) {
                        \Log::info("!!!======-----DailyUpdate deplicated save fail = ".$tripbus_key->id."-----======!!!");
                        continue;
                    }
                    ...
                    $daily_trip->save();
                    \Log::info("!!!======-----NewUpdate saved dailyTripDetail = ".$daily_trip->id."-----======!!!");
                }
            }
